...
const getHomeTimeline = (node, delay) => {
  ...
  const bgImage = node.querySelectorAll(
    ".Homepage .MainImageAndHeader, .Homepage .MainImageAndHeader .darken "
  );
  const header = node.querySelector(".Homepage h1");
  header.innerHTML =
    "<span>" + header.innerHTML.split("").join("</span><span>") + "</span>";
...
const getDefaultTimeline = (node, delay) => {
  ...
  const header = node
    .querySelector(".HorizontalBigCard")
...
const getBlogTimeline = (node, delay) => {
  ...
  const carousel1 = node.querySelectorAll(
    ".custom-indicator, .carousel-control-prev, .carousel-control-next"
  );
  timeline
    .from(node, 0, { display: "none", autoAlpha: 0, delay })
...
export const play = (pathname, node) => {
  ...
  else timeline = NoMatch(node, delay);
