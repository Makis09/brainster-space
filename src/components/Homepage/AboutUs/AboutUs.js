...
import CompanyInovationModal from '../../Forms/CompanyInovationModal';
import HorizontalBigCard from '../../Cards/HorizontalBigCard';

const AboutUs = () => {
    const { setShowPartnershipModal, setShowCompanyInovationModal } = useContext(MyContext);
    const [isEducation, setIsEducation] = useState(true);
    const educationInfo = {
        title: 'ЕДУКАЦИЈА',
        text: 'Дали се подготвен да одговориш на потребите на иднината. Вистинските курсеви, академии и семинари кои ќе ти овозможат кариерна трансформација во областа дигитален маркетинг, дизајн, програмирање и Data Science.',
        image: 'edukacija1'
    }
    const companiesInfo = {
        title: 'КОМПАНИИ',
        text: "Компаниите имаат можност да ги надоградат своите тимови, а со тоа да го подобрат перформансот на својата компанија. \n Дигиталната трансформација се случува, а вашите компании треба да бидат подготвени за да се адаптираат соодветно. Обуки, семинари, курсеви или team building? \n Во Brainster Space имаме специјално обучен тим кој е подготвен да ја насочи, адаптира и сподели својата експертиза со денешните потреби на компаниите.",
        image: 'companies'
    }

    const companiesInfoText = companiesInfo.text.split('\n').map((text, index) => (
    ...
    return (
        <Row className="AboutUs">
            ...
            <HorizontalBigCard style={isEducation ? {display: 'flex' } : { display: 'none' }}
            ...
            >
                <Button onClick={() => setIsEducation(true)} className='blackBtn'>АКАДЕМИИ</Button>
                <Button onClick={() => setIsEducation(false)} className='yellowBtn'>ЗА КОМПАНИИ</Button>
            </HorizontalBigCard>
            <HorizontalBigCard style={isEducation ? {display: 'none' } : { display: 'flex' }}
